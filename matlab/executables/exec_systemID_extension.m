...
finished = false;
sin_amplitude = 150; %[deg/s^2]
freq_range = 4:0.2:20; %Hz
set_time = 1.5; %s
Ts=1e-3;
%% File names
global ADXL_file_name;
filename = sprintf("systemID_data_lin\\sine_lin_bladedist=%.4f,freq_range=[%.4f,%.4f].mat",blade_distance,freq_range(1),freq_range(end));
ADXL_file_name = sprintf("systemID_data_lin\\ADXL_sine_lin_bladedist=%.4f,freq_range=[%.4f,%.4f]",blade_distance,freq_range(1),freq_range(end));
%% Generate trajectory
tic
[x_ref,idx] = generate_sine_sweep(sin_amplitude,freq_range,set_time,Ts);
toc
[a1,a2] = inverse_kinematics(blade_distance,0);
[traj_ext,traj_rot]=forward_kinematics(x_ref+a1,-x_ref+a2);
