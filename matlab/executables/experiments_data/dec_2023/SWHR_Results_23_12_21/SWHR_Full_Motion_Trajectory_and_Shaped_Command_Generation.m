...
addpath('./functions_for_traj_gen')

%%%% Variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Ts = 1e-3;          % Sampling time [s]
x_start = 500;        % Starting position [mm]
x_end = 580;  % Ending position [mm]
dwell = 0.5;        % Dwell time between each step [s]
...
% v_lim = 200;    % Velocity limit [mm/s]         default=600
% a_lim = 2000;   % Acceleration limit [mm/s^2]   default=1500
% j_lim = 5e4; % Jerk limit[mm/s^3]               default=1e4

% v_lim = 100;    % Velocity limit [mm/s]         default=600
% a_lim = 2000;   % Acceleration limit [mm/s^2]   default=1500
% j_lim = 5e4; % Jerk limit[mm/s^3]               default=1e4

traj_name = ['Full_Motion_',num2str(x_start),'_to_',num2str(x_end),'_V',num2str(v_lim),'_A',num2str(a_lim),'_J',num2str(j_lim)];

% Set B-spline Parameters
m = 5;              % B-Spline Degree
CtrlPntFs = 100;    % Control poinr freq: number of control points per second [Hz]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% add dwell time
t_ref = [t_ref, t_ref(end) + (1:dwell/Ts)*Ts];
x_ref = [x_ref, x_ref(end) * ones(1,dwell/Ts)];
...
load('Silicon_Wafer_Handling_Robot_Params_for_Experiments.mat','SWHR_Parameters','Exp_Parameters','K','B','K_individual','B_individual')
...
%% Save
% save(['./Trajectories/SWHR_Traj_',traj_name],'t_ref','x_ref','Ts')
